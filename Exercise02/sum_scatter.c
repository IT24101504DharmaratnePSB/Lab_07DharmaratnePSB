...
int main(int argc, char **argv) {
    ...
    /*
     * SCATTER: Root divides the array into equal chunks and sends
     * ONE chunk to each process (root keeps its own chunk too).
     * sendcount = chunk_size (elements sent to EACH process, not the total!)
     */
    MPI_Scatter(array, chunk_size, MPI_INT,
                local_chunk, chunk_size, MPI_INT,
                0, MPI_COMM_WORLD);

    /* Each process sums the chunk it received */
    ...
    /* Collect results using point-to-point communication (unchanged from Ex1) */
    if (rank != 0) {
